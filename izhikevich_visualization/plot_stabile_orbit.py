...
for j,(initial_n, initial_V) in enumerate(zip([0.4, 0.45, 0.5, 0.55, 0.65, 0.70,0.75,0.4, 0.5, 0.55, 0.65, 0.70,], [0, 20, 30, 40, 50, 60, 70, 0,8,15,20,24,] )):
...
for j,(initial_n, initial_V) in enumerate(zip([0.42,0.47,0.43,0.44], [6,4.9,4.8, 4.8] )):
    sol = integrate.solve_ivp(dvdt, tspan, [initial_V, initial_n], method = "Radau")
    ax1.plot(sol.y[0], sol.y[1], 'tab:cyan', alpha=0.5)
    derrivatives = _dvdt(sol.y[0], sol.y[1])
    points = np.vstack((sol.y[0], sol.y[1]))
    arrow = np.vstack((derrivatives[0], derrivatives[1]))
tspan = [0, 1.7]
t_eval = np.linspace(0, 1.7, 50)
for j,(initial_n, initial_V) in enumerate(zip([0.3,], [-10,] )):
    sol = integrate.solve_ivp(dvdt, tspan, [initial_V, initial_n], method = "Radau")
    ax1.plot(sol.y[0], sol.y[1], 'tab:cyan', alpha=0.5)
    derrivatives = _dvdt(sol.y[0], sol.y[1])
    points = np.vstack((sol.y[0], sol.y[1]))
...
for j,(initial_n, initial_V) in enumerate(zip([0.35], [-12] )):
    sol = integrate.solve_ivp(dvdt, tspan, [initial_V, initial_n], method = "Radau")
    ax1.plot(sol.y[0], sol.y[1], 'tab:cyan', alpha=0.5)
    derrivatives = _dvdt(sol.y[0], sol.y[1])
    points = np.vstack((sol.y[0], sol.y[1]))
